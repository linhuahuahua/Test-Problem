 ...
 * 		nr=N/100
 * 		delta=0.9
 * 		weight vector read from files
 * 		crossover operator:DE
 * 		mutation operator£ºPM
 *
 * Reference:
...
public class MOEAD extends Algorithm {
	...
	double[]   z_;					  // ideal point(the minimal point in current population)
...
//		functionType_ = "_PBI";
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		//read weight vectors from the path
		...
		// STEP 1.1. read weight vectors from file and Compute Euclidean distances between weight vectors and find T
		initUniformWeight();
		initNeighborhood();
	...
	/**
	 * Initialize the neighborhood matrix of subproblems, based on the Eudclidean
	...
	//vectors a=(x1,y1),b=(x2,y2),
	//this function return: x1*x2+y1*y2
	...
	//vector a=(x,y)
	//this function return: square root(x*x+y*y)
	...
	//functionType:1.Tch  2.mTch  3.PBI  4.WS
	double fitnessFunction(Solution individual, double[] lambda) {
		...
		if (functionType_.equals("_TCHE1")) {
			...
			for (int n = 0; n < problem_.getNumberOfObjectives(); n++)
				realB[n] = Math.abs(individual.getObjective(n) - (z_[n] + d1* lambda[n]));
