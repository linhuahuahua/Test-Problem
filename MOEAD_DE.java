...
package jmetal.metaheuristics.IRA;
...
//		functionType_ = "_PBI";
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		populationSize_ = ((Integer) this.getInputParameter("populationSize"))
				.intValue();
		//A=((Double)this.getInputParameter("A")).doubleValue();
...
//		nr_ = 20;
	    ...
	    delta_ = 1;
	    nr_ = (int) (0.1 * populationSize_);
				
		neighborhood_ = new int[populationSize_][T_];

		z_ = new double[problem_.getNumberOfObjectives()];
		z_old = new double[problem_.getNumberOfObjectives()];
		z_ut = new double[problem_.getNumberOfObjectives()];
		//z_new=new double[populationSize_][problem_.getNumberOfObjectives()];
		...
		do {
			...
			double r1=(1-Math.pow((double)g*populationSize_/(maxEvaluations*0.9),A))*C;
			r=(r1>0)?r1:0;
...
//					sum=sum+Math.abs(z_[i]-z_old[i]);
//					z_old[i]=z_[i];
//				}
//				if (sum/problem_.getNumberOfObjectives()==0){
//					r=0;
//				}
//				else{
//					r=1;
//				}
//				System.out.println("g="+g+","+"sum="+sum+",r="+r+"  z1="+z_[0]+"  z2="+z_[1]);
//			}
//			
	...
	double fitnessFunction(Solution individual, double[] lambda) {
		double fitness;
		fitness = 0.0;
		double[] z_new=new double[problem_.getNumberOfObjectives()];
		double min_lambda=1.0e+3;
		for(int i=0;i<problem_.getNumberOfObjectives();i++){
		...
		for(int i=0;i<problem_.getNumberOfObjectives();i++){
			z_new[i]=z_[i]+(lambda[i]-min_lambda)*r;
			
			//z_new[i]=z_[i]+(lambda[i]-min_lambda)*individual.getObjective(i);
		}
		...
		if (functionType_.equals("_TCHE1")) {
			...
			for (int n = 0; n < problem_.getNumberOfObjectives(); n++) {
				double diff = individual.getObjective(n) - z_new[n];
			...
			for (int i = 0; i < problem_.getNumberOfObjectives(); i++) {
				...
				double diff = individual.getObjective(i) - z_new[i];
			...
			fitness = maxFun;
		}else if (functionType_.equals("_TCHE_n")) {
